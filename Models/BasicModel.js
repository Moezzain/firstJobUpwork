    ...
    return new Promise(function(resolve, reject) {
      DB.query(qr, [rowID, rowID, rowID, rowID, rowID, rowID], function(err, result) {
        ...
        result[4].forEach((element, index) => {
          if ('Democratic_Inc_Lead' === element["case_dim.dimension"])
            ret['progressDataForEachDimenstion']['dates'].push(element.date);

          if (!ret['progressDataForEachDimenstion'][element["case_dim.dimension"]])
            ret['progressDataForEachDimenstion'][element["case_dim.dimension"]] = [];
          ret['progressDataForEachDimenstion'][element["case_dim.dimension"]].push(
            element.Average,
          );
        });
        // console.log("progressDataForEachDimenstion " + JSON.stringify(ret["progressDataForEachDimenstion"]));
